...
export default defineConfig({
  plugins: [
    // Fix: Vite 7 drops vuetify's virtual SASS modules from the graph on page reload.
    // The vuetify:styles plugin's `tempFiles` Map is empty because `resolveId` (which
    // populates it) hasn't been called yet when the browser directly requests the virtual
    // module URL. We catch that in `load` and regenerate the SASS @use content ourselves.
    {
      name: 'fix-vuetify-virtual-resolve',
      resolveId(source) {
        if (source.startsWith('virtual:plugin-vuetify:'))
          return source
      },
      async load(id) {
        if (id.startsWith('virtual:plugin-vuetify:')) {
          const relativePath = id.replace('virtual:plugin-vuetify:', '').replace(/\?.*$/, '')
          const configFile = fileURLToPath(new URL('./resources/styles/variables/_vuetify.scss', import.meta.url))
          const vuetifyLibDir = fileURLToPath(new URL('./node_modules/vuetify/lib', import.meta.url))
          const sassFile = path.resolve(vuetifyLibDir, relativePath)
          const suffix = relativePath.endsWith('.scss') ? ';\n' : '\n'
          const normalizePath = p => {
            p = path.normalize(p).replace(/\\/g, '/')
            return /^[a-z]:\//i.test(p) ? '/' + p : p
          }
          return `@use "${normalizePath(configFile)}"${suffix}@use "${normalizePath(sassFile)}"${suffix}`
        }
        return null
      },
    },
